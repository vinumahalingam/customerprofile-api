...
public class CustomerProfileServiceImpl implements CustomerProfileService {
    ...
    public CustomerProfileDTO findByID(String id) throws CustomerProfileNotFoundException {
        try {
            ...
            CustomerProfile profile = customerProfileRepository.findById(profileId)
                    .orElseThrow(
                            () -> new CustomerProfileNotFoundException("Customer profile not found with id: " + id));
    ...
    @Override
    public List<CustomerProfileDTO> findByFullName(String firstName, String lastName)
        ...
        if (profiles.isEmpty()) {
            throw new CustomerProfileNotFoundException(
                    "No customer profiles found with name: " + firstName + " " + lastName);
        }
        return profiles.stream()
                .map(this::mapToDTO)
                .collect(Collectors.toList());
    ...
    @Override
    public CustomerProfileDTO createCustomerProfile(CustomerProfileRequest request)
            throws CustomerProfileNotFoundException {
        ...
        if (customerProfileRepository.findByEmail(request.getEmail()).isPresent()) {
            throw new CustomerProfileNotFoundException(
                    "Customer profile already exists with email: " + request.getEmail());
        }

        String accountStatus = request.getAccountStatus() != null ? request.getAccountStatus().trim().toLowerCase()
                : "active";
        if (!accountStatus.equals("active") && !accountStatus.equals("suspended")
        ...
        try {
            ...
            throw new CustomerProfileNotFoundException(
                    "Error creating customer profile: " + e.getRootCause().getMessage());
        }
    }

    @Override
    public CustomerProfileDTO updateCustomerProfile(String id, CustomerProfileRequest request)
            throws CustomerProfileNotFoundException {
        if (request == null) {
        ...
        try {
            ...
            if (request.getAccountStatus() != null) {
                ...
                if (!accountStatus.equals("active") && !accountStatus.equals("suspended")
                        && !accountStatus.equals("inactive")) {
                    throw new CustomerProfileNotFoundException(
                            "accountStatus must be 'active', 'suspended', or 'inactive'");
                }
                profile.setAccountStatus(accountStatus);
            }
            ...
            throw new CustomerProfileNotFoundException(
                    "Error updating customer profile: " + e.getRootCause().getMessage());
        }
    }

    @Override
    public void deleteCustomerProfile(String id) throws CustomerProfileNotFoundException {
        try {
            Long profileId = Long.parseLong(id);
            if (!customerProfileRepository.existsById(profileId)) {
                throw new CustomerProfileNotFoundException("Customer profile not found with id: " + id);
            }
